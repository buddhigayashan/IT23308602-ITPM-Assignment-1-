...
def _ensure_columns_after(ws, header_row: int, anchor_col: int, header_values: list, desired_names: list[str]) -> list[int]:
    existing = []
    for name in desired_names:
        found = _find_column_index(header_values, name, [])
        if found:
            existing.append(found)
        else:
            existing.append(None)

    missing_count = sum(1 for col in existing if col is None)
    if missing_count == 0:
        return [int(col) for col in existing]

    insert_at = anchor_col + 1
    ws.insert_cols(insert_at, missing_count)

    for offset, name in enumerate(desired_names, start=0):
        ws.cell(row=header_row, column=insert_at + offset).value = name

    return [insert_at + i for i in range(missing_count)]
